            ...
            <form method="POST" action="{{ route('expense.update', $expense) }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $expense->date->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Amount (₹) *</label>
                        <input type="number" name="amount" step="0.01" value="{{ old('amount', $expense->amount) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Payment Mode</label>
                        <select name="payment_mode" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                            <option value="">Select</option>
                            @foreach(['Cash','Bank Transfer','UPI','Credit Card','Cheque'] as $mode)
                                <option value="{{ $mode }}" @selected(old('payment_mode', $expense->payment_mode) == $mode)>{{ $mode }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="description" rows="2" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">{{ old('description', $expense->description) }}</textarea>
                    </div>
                </div>
                <div class="flex gap-3">
                    <button type="submit" class="bg-red-500 text-white px-6 py-2 rounded-lg font-medium hover:bg-red-600">Update Expense</button>
